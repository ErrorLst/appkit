...
class Config final {
  ...
  public:
    ...
    [[nodiscard]] static auto parse(std::string_view text, ConfigError &error) -> std::optional<Config>;
    ...
    /// 指定 (section, key) 是否存在。
    [[nodiscard]] auto has(std::string_view section, std::string_view key) const noexcept -> bool;

    /// 已解析的唯一键数量。
    [[nodiscard]] auto size() const noexcept -> std::size_t;

    /// 展开值中的 ${...} 引用；失败时写 error 并返回 nullopt。
    [[nodiscard]] auto expand(std::string_view section, std::string_view key, ConfigError &error) const
        -> std::optional<std::string>;

    /// 按 (section, key) 查找条目；未命中返回 nullptr。
    /// 返回的指针指向内部存储，在 Config 存活期内有效。
    template <TextArg S, TextArg K> [[nodiscard]] auto find(S section, K key) const noexcept -> const Entry * {
